...
_URL = "https://www.darkmap.cn/api/lightpollution/latest"
...
async def fetch_light_pollution(lat: float, lon: float) -> LightPollution:
    """Fetch the latest sky brightness at *(lat, lon)* from DarkMap."""
    _validate_coordinates(lat, lon)
    async with httpx.AsyncClient(timeout=15.0, follow_redirects=True) as client:
        res = await client.get(_URL, params={"lat": lat, "lon": lon})
