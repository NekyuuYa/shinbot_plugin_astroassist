# AstroAssist 晴天钟助手 🔭

天文爱好者专属的气象预报插件，基于 **ShinBot** 框架开发。调用 **Open-Meteo** 的 ECMWF 高精度数据与 **7Timer!** 专业天文指标，输出看板式 PNG 预报图。

## ✨ 核心特性

- **专业气象指标**：总云量、低/中/高空云量、气温、露点温度、相对湿度、风速。
- **天文指标**：视宁度 (Seeing) 与透明度 (Transparency)，来自 7Timer! API。
- **结露风险预警**：根据露点温度自动标记风险等级。
- **高德地图集成**：支持地名定位，内置 GCJ-02 → WGS-84 坐标转换。
- **日夜主题切换**：根据目标位置日出日落自动切换浅色/深色看板。
- **超清渲染**：3 倍超采样输出 2100px 物理宽度 PNG 原图（依赖 RenderKit）。
- **雷达回波**：实时获取中央气象台全国雷达回波拼图。
- **雷达临近站点**：输入没有独立雷达站的城市时，读取 NMC 省级站点导航，并通过高德地理编码按距离推荐或选择临近站点。
- **海区云图**：获取中央气象台 NMC 海区红外云图，并作为台风信息补充图。
- **台风快讯**：获取中央气象台 NMC 最新台风快讯摘要、详情、路径预报图与 Dapiya 台风云图。
- **光污染报告**：调用 DarkMap 接口生成 Bortle 等级、SQM 天光亮度、极限星等与银河可见性报告。
- **卫星过境预报**：拉取 CelesTrak TLE 轨道数据，本地 SGP4 推算国际空间站/天宫空间站/哈勃的夜间过境时刻、高度角、方位与亮度。

## 🚀 安装

1. 确保已安装 `shinbot_plugin_renderkit`（提供 HTML 渲染能力）。
   AstroAssist 会通过自身 `pyproject.toml` 同步 `playwright` Python 依赖。
2. 在 ShinBot 配置中启用本插件。
3. 配置高德地图 API Key（Web服务类型）：
   ```toml
   [[plugins]]
   id = "shinbot_plugin_astroassist"
   enabled = true

   [plugins.config]
   amap_key = "your_amap_api_key"
   ```

## 🎮 指令

| 指令 | 别名 | 说明 |
|---|---|---|
| `!晴天钟` | `!astro` | 获取看板预报 |
| `!晴天钟 -d 1` | | 指定天数 (1-7) |
| `!晴天钟 -n` | | 仅夜间时段 |
| `!晴天钟 西藏阿里` | | 临时查询某地 |
| `!设置位置 北京天文馆` | `!setloc` | 设置默认观测位置 |
| `!设置位置 -c 39.9 116.4` | | 手动坐标 (WGS-84) |
| `!雷达` | `!radar` | 获取最新全国雷达回波拼图 |
| `!雷达动图` | `!radargif` | 全国雷达回波动图 (~2小时动画) |
| `!雷达 华北` | | 区域拼图 (华北/华东/华中/华南/西南/西北/东北) |
| `!雷达 北京` | | 单站雷达 (省份名或城市名) |
| `!雷达 无锡` | | NMC 无锡无单独站点时自动使用常州等临近站点，并显示距离；其他地点可推荐最近站点 |
| `!海区云图` | `!seacloud` | 获取最新海区红外云图 |
| `!海区云图 西北太平洋` | | 西北太平洋海区红外云图 |
| `!海区云图动图` | `!seacloudgif` | 海区红外云图动画 |
| `!台风` | `!typhoon` | 查询中央气象台最新台风快讯 |
| `!台风 <名称或编号>` | | 查询任一活跃台风详情并附带路径预报图、Dapiya 台风云图 |
| `!台风云图 巴威` | `!typhooncloud` | Dapiya 台风云图，默认 VIS |
| `!台风云图 09W RGB` | | 指定云图类型 (VIS/RGB/TRUECOLOR) |
| `!台风云图动图 BAVI TRUECOLOR` | `!typhooncloudgif` | Dapiya 台风云图动画 |
| `!台风 help` | | 显示台风查询帮助 |
| `!光污染 [地名] [年份]` | `!lightpollution` `!bortle` `!光害` | 生成当前位置光污染 (Bortle) 报告，含 SQM/极限星等/银河可见性；年份 2012-2025 逐年对比 |
| `!过境卫星` | `!卫星过境` `!transit` `!satpass` | 预报默认位置未来 3 天国际空间站/天宫空间站过境时刻（含高度角/方位/亮度，默认仅夜间） |
| `!过境卫星 哈勃` | | 指定卫星（国际空间站/天宫空间站/哈勃，或 NORAD 编号） |
| `!过境卫星 -d 5` | | 指定天数 (1-7) |
| `!过境卫星 上海` | | 临时查询某地 |
| `!晴天钟 help` | | 显示帮助信息 |

## 📊 视觉说明

- **气温/风速**：色阶方块，颜色越深越极端。
- **露点风险**：🟢安全 / 🟡中风险 / 🔴高危（易结露）。
- **云量**：日间蓝天底 + 白色填充；夜间深蓝底 + 灰色填充。

## 🛠️ 开源协议

MIT License。数据源由 [Open-Meteo](https://open-meteo.com/)、[7Timer!](http://www.7timer.info/)、[中央气象台 NMC](http://www.nmc.cn/)、[DarkMap](https://www.darkmap.cn/) 和 [CelesTrak](https://celestrak.org/) 提供。
